drop table sale;
drop table sale_car;
drop materialized view mv_sale_cars;
...
create materialized view mv_sale_cars
...
select * from mv_sale_cars;

-- смотрим план запроса. Он должен измениться на mview
select s.id, s.sale_date, SUM(sc.summa) total_summa
 from sale s
 join sale_car sc on sc.sale_id = s.id
 group by s.id, s.sale_date;
